...
SELECT F.*, C.CNAME FROM FILEBOARD F, CUSTOMER C WHERE F.CID=C.CID ORDER BY fREF DESC, fRE_STEP; -- 기준
SELECT * FROM (SELECT ROWNUM RN, A.* FROM (SELECT F.*, C.CNAME FROM FILEBOARD F, CUSTOMER C WHERE F.CID=C.CID ORDER BY fREF DESC, fRE_STEP )A) 
