...
public class BoardDao {
	private DataSource ds; // connection 에 쌓아두기로 했던 데이터 타입이 DataSource
	public BoardDao() {
		try {
	...
	// 글 쓰기 // bname, btitle, bcontent, bip
	public int write(String bname, String btitle, String bcontent, String bip) {
		...
		try {
			...
			System.out.println(result==1?"글 쓰기 성공":"글 쓰기 실패");
		}catch(Exception e) {
			System.out.print(e.getMessage());
		}finally {
			try {
	...
	// 답변 글 쓰기 - 답변자 : bname, btitle, bcontent, bip / 원글 : bgroup, bstep, bindent
	public int reply(String bname, String btitle, String bcontent, int bgroup, int bstep, int bindent, String bip) {
		...
		try {
			...
			result = pstmt.executeUpdate();
			System.out.println(result==1?"답변 글 쓰기 성공":"답변 글 쓰기 실패");
		}catch(Exception e) {
			System.out.print(e.getMessage());
		}finally {
			try {
	...
	// 글 수정 - bname, btitle, bcontent, bip, bid
