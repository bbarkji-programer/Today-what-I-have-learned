...
<%
	...
	BoardDto dto = new BoardDto(); // 답변글을 위해 들어온 경우, dto에 원글에 대한 정보를 담는다. 
	// 답변글을 쓰려고 온 경우 dto에 원글에 대한 정보로
	if(request.getParameter("num")!=null){ // num으로 원글 정보 가져옴
		num = Integer.parseInt(request.getParameter("num"));
		BoardDao bDao = BoardDao.getInstance();
		dto = bDao.getBoardOneLine(num); // dto에 num으로 가져온 원글 정보를 넣음.
		ref = dto.getRef();
